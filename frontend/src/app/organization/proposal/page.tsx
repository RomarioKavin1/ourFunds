...
import { handleVoteAttestation } from "@/utils/signProtocol";
...
const ProposalDetail = () => {
  ...
  const [proposalDescription, setProposalDescription] = useState(
    exampleProposal.description
  );
  const [proposalCreatorAddress, setProposalCreatorAddress] = useState(
    exampleProposal.creatorAddress
  );
  const [proposalDateCreated, setProposalDateCreated] = useState(
    exampleProposal.dateCreated
  );
  const [proposalStatus, setProposalStatus] = useState(exampleProposal.status);
  const [proposalNoOfStackHolders, setProposalNoOfStackHolders] = useState(
    exampleProposal.noOfStackHolders
  );
  ...
  const handleSubmit = async () => {
    ...
    await handleVoteAttestation({
      setLoading: setIsLoading,
      setError: console.error,
      setAttestationInfo: console.log,
  ...
  const renderStatusContent = () => {
    switch (proposalStatus) {
      case "Vote Pending":
        return (
          <div
            className="border flex flex-col justify-center items-center h-auto p-52 w-3/5 gap-y-24 rounded-[20px] 
                    z-10 bg-[white]/[0.04] hover:bg-[white]/[0.08] border-[white]/[0.08]"
            onClick={handleClick}
          >
            <p className="w-3/ text-lg text-center">
              It seems that you have not voted yet, cast your vote for the
              proposal which will be aggregated with votes for other
              stakeholders
            </p>
            <img
              src={CastVote.src}
      ...
      case "Approved":
        return (
          ...
          >
            <p className="w-3/ text-lg text-center">
              The proposal has been approved. Thank you for your vote!
            </p>
            <img
              src={CheckMark.src}
              style={{ width: "100px", height: "100px" }}
      ...
      case "Rejected":
        return (
          ...
          >
            <p className="w-3/ text-lg text-center">
              The proposal has been rejected. Thank you for your vote!
            </p>
            <img
              src={Cancel.src}
              style={{ width: "100px", height: "100px" }}
      ...
      case "Waiting":
        return (
          ...
          >
            <p className="w-3/ text-lg text-center">
              Waiting for the votes of other stackholders. Please check back
              later.
            </p>
            <img
              src={Waiting.src}
      ...
      >
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col justify-center items-center gap-y-10">
            <h1 className="text-6xl text-third font-extrabold mb-8 z-10">
              {proposalTitle}
            </h1>
            ...
            >
              ...
              >
                ...
                >
                  Powered By{" "}
                  <img src="/signprotocol.avif" className="ml-2 w-16" />
                </Typography>
              </Box>
            </Modal>
          </div>
